...
import { COVE_V1_MAINNET_CONFIG, configDomain } from "./config.js";
import { createCoveState, type CoveTransaction } from "./types.js";
import mainnetVectors from "./state-root-mainnet-vectors.json";

const CFG = COVE_V1_MAINNET_CONFIG;
...
function deploy(): CoveTransaction {
  return {
    ...
    ticker: "FROG",
    protocolOutputs: [
      { index: 1, scriptPubKeyHex: "0014" + "22".repeat(20), amountSats: 10_000n, role: "launch-fee" },
    ],
  };
}
...
describe("mainnet state-root vectors (frozen, unactivated domain)", () => {
  it("domain matches the frozen mainnet domain", () => {
    expect(DOMAIN).toBe(mainnetVectors.domain);
  });

  for (const vector of mainnetVectors.vectors) {
    it(`matches the frozen mainnet golden root for "${vector.id}"`, () => {
