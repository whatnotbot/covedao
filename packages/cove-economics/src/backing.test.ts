...
describe("requiredBackingSats R(s) golden vectors (stairs210)", () => {
  it("R(0) = 0, R(100k) = 2,700, R(200k) = 8,100", () => {
    expect(requiredBackingSats(0n)).toBe(0n);
    expect(requiredBackingSats(STAGE)).toBe(2_700n);
    expect(requiredBackingSats(2n * STAGE)).toBe(8_100n);
  });
  it("R(21M) = 59,818,500 sats (full raise)", () => {
    expect(requiredBackingSats(PUBLIC_SUPPLY)).toBe(59_818_500n);
  });
});

describe("quoteBuy / quoteRedeem golden vectors", () => {
  it("buy one stair from 0 = 2,700 gross, 6,203 fee (5,000 + 100 lots × 10 + 7.5%)", () => {
    expect(quoteBuy(0n, STAGE)).toEqual({ gross: 2_700n, fee: 6_203n, net: 8_903n });
  });
  it("redeem one stair from one stair = 2,700 gross, 1,000 fee (7.5%, floored at 1,000)", () => {
    expect(quoteRedeem(STAGE, STAGE)).toEqual({ gross: 2_700n, fee: 1_000n, net: 1_700n });
  });
...
describe("P0 invariant: buy→redeem round trip cannot drain backing", () => {
  ...
  it("redemption reduces issued supply and releases capacity for re-buy", () => {
    ...
    expect(requiredBackingSats(afterRedeem)).toBe(59_818_500n - redeemGross);
...
describe("boundary / failure cases", () => {
  ...
  it("single-unit quantities work", () => {
    // 1 token at 27 sats a lot is 0.027 sats, rounded up.
